...
import { getCurrencies, getAllCurrencies, currencyBuy } from './api.js';
import Choices from 'choices.js';
import { createInput } from './login-page.js';
import { createError } from './errors.js';

async function createMyCurrencyBlock(container, token) {
	const $myCurrenciesSkeleton = el(
		'.currency__my-currencies-skeleton.my-currencies-skeleton'
	);
	mount(container, $myCurrenciesSkeleton);
	...
	if (currenciesArray.error) {
		...
		unmount(container, $myCurrenciesSkeleton);
...
async function createExchangeBlock(container, token) {
	...
	const $exchangeSkeleton = el(
		'.currency__exchange-skeleton.exchange-skeleton'
	);
	mount(container, $exchangeSkeleton);
	...
	if (currnciesArr.error) {
		...
		unmount(container, $exchangeSkeleton);
...
function createRealTimeRatesBlock(container) {
	...
	const $realTimeRatesSkeleton = el(
		'.currency__real-time-rates-skeleton.real-time-rates-skeleton'
	);
	mount(container, $realTimeRatesSkeleton);
	...
	socket.addEventListener('open', () => {
		unmount(container, $realTimeRatesSkeleton);
		mount(container, $block);
	...
	socket.addEventListener('message', (event) => {
		...
		if (message.change == 1) {
			$currencyTr.classList.add('increase');
		} else {
			$currencyTr.classList.add('decrease');
		}
