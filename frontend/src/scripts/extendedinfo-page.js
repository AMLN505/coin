...
import { transfer, getInfoAboutAccount } from './api.js';
import { createPayHistoryPage } from './payhistory-page.js';
import { Dynamic } from './dynamic-class.js';
...
function createTransferBlock(accNumber, container, className, token) {
	...
	$block.addEventListener('submit', async (e) => {
		...
		if (response.error) {
			...
			createExtendedInfoPage(accNumber);
		}
	});
}
...
export async function createExtendedInfoPage(accNumber) {
	...
	const $navSkeleton = el('.extended-info__nav-skeleton.nav-skeleton');
	...
	const $dynamicSkeleton = el(
		'.extended-info__dynamic-skeleton.dynamic-skeleton'
	);
	const $transSkeleton = el(
		'.extended-info__transactions-skeleton.transactions-skeleton'
	);
	mount($grid, $dynamicSkeleton);
	mount($grid, $transSkeleton);
		...
		if (response.payload.transactions.length != 0) {
			...
			if ($dynamicBlock) {
				$dynamicBlock.addEventListener('click', () => {
					router.navigate(
						`/accounts/${response.payload.account}/history`
					);
				});
			}
			...
			router.on(`/accounts/${response.payload.account}/history`, () => {
				createPayHistoryPage(response.payload.account);
			});
		} else {
			unmount($grid, $dynamicSkeleton);
			unmount($grid, $transSkeleton);
